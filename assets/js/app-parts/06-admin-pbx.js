...
function initAdmin(){adminTab('main');setToday();setCurMonth();updateDates();updateHolList();updateIPList();loadGPSSettingsUI();loadFaceControlSettings();loadMyIPDisplay();loadNotifEmpSelect();checkFeedbackBadge();subscribeAdminRealtime();loadTodayWorkLeaveCard();startPbxAutoRefresh();}
...
function adminTab(tab){
  currentAdminTab = tab;
  ['main','notif','feedback','settings','kpi','amocrm','pbx'].forEach(n=>{const el=document.getElementById('tab_'+n);if(el)el.classList.toggle('hidden',n!==tab);});
  ['nav_main','nav_amocrm','nav_feedback','nav_notif','nav_settings','nav_kpi','nav_pbx'].forEach((id,i)=>{const el=document.getElementById(id);if(el)el.classList.toggle('active',['main','amocrm','feedback','notif','settings','kpi','pbx'][i]===tab);});
  if(tab==='pbx') loadPbxStats();
  ...
  if(tab==='amocrm')startAmoCrmTaskPage();
  animateCurrentView();
}
...
function renderPbxCalendar(){
  ...
  for(let i=0;i<42;i++){
    ...
    btn.onclick=()=>selectPbxCalendarDate(iso);
...
function computePbxStats(calls=[], apiStats={}){
  ...
  for(const call of calls){
    ...
    if(isIncoming)computed.incoming+=1;
    if(isOutgoing)computed.outgoing+=1;
    if(isInternal)computed.internal+=1;
    if(isMissed)computed.missed+=1;
    if(!isMissed && duration>0)computed.answered+=1;

    computed.totalDuration+=duration;

    if(onlinePbxNumber(raw.quality_score || call.quality_score)>0)computed.withQualityScore+=1;
    if(accountcode.includes('callback') || status.includes('callback'))computed.callback+=1;
    if(isOutgoing && duration===0)computed.notReached+=1;
    if(isIncoming && isMissed && !raw.callback_at && !raw.called_back_at)computed.notCalledBack+=1;
  }
...
async function fetchOnlinePbxAllCalls(filters={}){
  const allCalls=[];
  const baseFilters={...ONLINE_PBX_DEFAULT_FILTERS,...(filters||{}),course:'all',limit:ONLINE_PBX_CHART_PAGE_LIMIT,offset:0};
  let offset=0;
  let lastData=null;
...
function getPbxCourseChartRows(calls=[]){
  const rows=new Map(Object.entries(PBX_COURSE_MAP).map(([code,label])=>[
    code,
    { code, label, total:0, talked:0, incoming:0, outgoing:0 }
  ]));
  const unknown={ code:'unknown', label:'Aniqlanmadi', total:0, talked:0, incoming:0, outgoing:0 };

  (calls||[]).forEach(call=>{
    const info=getPbxCourseInfo(call);
    const row=rows.get(info.code) || unknown;
    const direction=getOnlinePbxDirectionInfo(call.direction).key;
    const duration=onlinePbxNumber(call.duration);
    row.total+=1;
    if(duration>0)row.talked+=1;
    if(direction==='incoming')row.incoming+=1;
    if(direction==='outgoing')row.outgoing+=1;
  });
...
function renderPbxCourseChart(calls=[]){
  ...
  const talked=rows.reduce((sum,row)=>sum+row.talked,0);
  st('pbx_course_chart_note', onlinePbxFormatNumber(total) + ' ta qo\'ng\'iroq, ' + onlinePbxFormatNumber(talked) + ' ta gaplashilgan');
  chart.innerHTML='';
  ...
  rows.forEach(row=>{
    ...
    const talkedPct=row.total ? Math.round((row.talked/row.total)*100) : 0;
    const line=document.createElement('div');
    line.className='pbx-course-chart-row' + (row.total ? '' : ' empty');
    line.innerHTML=[
      '<div class="pbx-course-chart-label">',
      '<strong>' + escapePbxHtml(row.code === 'unknown' ? row.label : row.code + ' - ' + row.label) + '</strong>',
      '<small>' + talkedPct + '% gaplashilgan</small>',
      '</div>',
      '<div class="pbx-course-chart-bar"><span style="width:' + pct + '%"></span></div>',
      '<div class="pbx-course-chart-metrics">',
      '<span><b>' + onlinePbxFormatNumber(row.total) + '</b> jami</span>',
      '<span><b>' + onlinePbxFormatNumber(row.talked) + '</b> gaplashilgan</span>',
      '<span><b>' + onlinePbxFormatNumber(row.incoming) + '</b> kiruvchi</span>',
      '<span><b>' + onlinePbxFormatNumber(row.outgoing) + '</b> chiquvchi</span>',
      '</div>'
    ].join('');
...
function renderPbxHourChart(calls=[]){
  ...
  const max=Math.max(...rows.map(row=>row.total),1);
  st('pbx_hour_chart_note', total ? 'Eng ko\'p: ' + String(peak.hour).padStart(2,'0') + ':00 (' + onlinePbxFormatNumber(peak.total) + ')' : '-');
  chart.innerHTML='';
  ...
  rows.forEach(row=>{
    ...
    bar.innerHTML=[
      ...
      '<div class="pbx-hour-label">' + String(row.hour).padStart(2,'0') + '</div>'
    ].join('');
    bar.title=String(row.hour).padStart(2,'0') + ':00 - ' + onlinePbxFormatNumber(row.total) + ' ta, ' + onlinePbxFormatNumber(row.talked) + ' gaplashilgan';
...
async function loadPbxStats(options={}){
  ...
  try{
    ...
    try{
      ...
      renderPbxCharts(filterCallsByCourse(fullData.calls || [], course));
    }catch(chartErr){
      if(requestId!==onlinePbxPageState.requestId)return;
...
function setAttendanceTableMode(monthly){
  ...
  if(monthly){
    ...
    st('th6', t('mas_late_count'));
    st('th7', t('mas_absent_days'));
    st('th_afk', t('mas_late_time'));
      ...
      bars.innerHTML=sortedRows.map(x=>`
        <div class="dash-eff-card" title="${x.name}: ${x.efficiency}%">
          ...
          <div class="dash-eff-breakdown">
            ...
            <div class="dash-eff-row">
              <div class="dash-eff-label"><span class="dash-eff-dot yellow"></span><span>${t('eff_kpi')}</span></div>
              <div class="dash-eff-value">${x.kpiPct}%</div>
            </div>
            <div class="dash-eff-row">
...
async function loadDashboard(){
  ...
  const [{data:emps,error:empErr},{data:recs,error:recErr},{data:bons,error:bonErr},leaves] = await Promise.all([
    sb.from('employees').select('*'),
    sb.from('attendance').select('*').gte('work_date',monthStart).lte('work_date',monthEnd),
    sb.from('kpi_bonuses').select('*').eq('month',month),
    fetchEmployeeLeavesInRange(monthStart, monthEnd)
  ]);
  if(empErr||recErr||bonErr){toast('error','Dashboard',(empErr||recErr||bonErr).message||t('dashboard_error'));return;}
  ...
  const rows=(emps||[]).map(emp=>{
    ...
    const bon=(bons||[]).find(b=>b.employee_id===emp.id)?.bonus||0;
    const pen=late*10+abs*20;
    const ball=Math.min(100,Math.max(0,100-pen+bon));
    const kpi=calcKpi(ball,emp.kpi_type||'senior');
    const expectedSec=Math.max(1,wd*TARGET_WORK_SEC_PER_DAY);
    const workPct=Math.min(100,Math.round((sec/expectedSec)*100));
    const kpiPct=Math.max(0,Math.min(100,ball));
    ...
    const efficiency=Math.max(0,Math.min(100,Math.round((workPct*0.55)+(kpiPct*0.45)-latePenalty-afkPenalty)));
    return {name:emp.name||'-',sec,late,lateMin,kpi,ball,came,abs,efficiency,workPct,kpiPct,latePenalty,afkPenalty};
  });
  ...
  if (dateFrom === dateTo) {
    ...
    Object.values(grouped).forEach((emp, i) => {
      ...
      tr.innerHTML = `
        ...
        <td>${emp.totalLate > 0 ? emp.totalLate + ' ' + t('daq') : '-'}</td>
        <td>${hrs2}</td>
...
async function loadMonthly(){
  ...
  const monthEnd=`${month}-${String(dim).padStart(2,'0')}`;
  const [{data:emps,error:empErr},{data:recs,error:recErr},{data:bons,error:bonErr},leaves] = await Promise.all([
    sb.from('employees').select('id,name,login,kpi_type').eq('active',true).order('name'),
    sb.from('attendance').select('employee_id,work_date,start_time,end_time,work_seconds,afk_seconds,late_minutes').gte('work_date',monthStart).lte('work_date',monthEnd),
    sb.from('kpi_bonuses').select('employee_id,bonus').eq('month',month),
    fetchEmployeeLeavesInRange(monthStart, monthEnd)
  ]);
  if(empErr||recErr||bonErr||!emps)return;
  const leaveDateMap=buildEmployeeLeaveDateMap(leaves, monthStart, monthEnd);
  ...
  const bonusMap=new Map((bons||[]).map(b=>[b.employee_id,b.bonus||0]));
  let tL=0,tA=0,tH=0,tK=0;
  monthlyData=[];
  ...
  emps.forEach((emp,i)=>{
    ...
    const bon=bonusMap.get(emp.id)||0;
    const pen=late*10+abs*20;
    const fin=Math.min(100,Math.max(0,100-pen+bon));
    const kpi=calcKpi(fin,emp.kpi_type||'senior');
    tL+=late;tA+=abs;tH+=sec;tK+=kpi;
    const hs=formatHm(sec);
    monthlyData.push({name:emp.name,hours:hs,afk:formatPenaltyAfk(afkSec),late,half,abs,penalty:-pen,bonus:bon,ball:fin,kpi});
    ...
    tr.innerHTML=`<td>${i+1}</td><td>${emp.name}</td><td style="font-family:var(--mono)">${hs}</td><td style="color:${afkSec>0?'var(--danger)':'var(--text2)'};">${formatPenaltyAfk(afkSec)}</td><td>${late} ${t('marta')}</td><td>${half} ${t('kun')}</td><td>${abs} ${t('kun')}</td><td style="color:var(--danger)">-${pen} ${t('ball')}</td><td style="color:var(--success)">+${bon} ${t('ball')}</td><td><strong style="font-family:var(--mono)">${fin} ${t('ball')}</strong></td><td style="color:var(--success);font-weight:700;font-family:var(--mono)">${kpi.toLocaleString()} UZS</td><td><div style="display:flex;gap:3px"><button class="bsm bsm-g" onclick="adjKpi('${emp.id}','${month}',10)">+10</button><button class="bsm bsm-r" onclick="adjKpi('${emp.id}','${month}',-10)">-10</button><button class="bsm bsm-gr" onclick="adjKpi('${emp.id}','${month}',0,true)">${t('reset')}</button></div></td>`;
    tbody.appendChild(tr);
  ...
  st('me',''+emps.length);st('ml',''+tL);st('ma',''+tA);st('mh',formatHm(tH));st('mk',tK.toLocaleString()+' UZS');
  filterAtt();
}

function calcKpi(ball,type){if(type==='senior'){if(ball>=90)return 560000;if(ball>=80)return 420000;if(ball>=70)return 280000;return 0;}if(ball>=90)return 400000;if(ball>=80)return 300000;if(ball>=70)return 210000;return 0;}
...
async function adjKpi(eid,month,delta,reset=false){const{data:ex}=await sb.from('kpi_bonuses').select('bonus').eq('employee_id',eid).eq('month',month).maybeSingle();const bonus=reset?0:((ex?.bonus||0)+delta);await sb.from('kpi_bonuses').upsert({employee_id:eid,month,bonus},{onConflict:'employee_id,month'});loadMonthly();loadDashboard();}

async function loadEmpList() {
  const { data: emps, error } = await sb
    .from('employees')
    .select('id,name,login,kpi_type,face_registered,responsible_course')
    .eq('active', true)
    .order('name');
  ...
  emps.forEach((emp, i) => {
    const ka = emp.kpi_type === 'junior' ? '400 000' : '560 000';
    const courseKey = getEmployeeCourseResponsibilityKey(emp);
    const courseLabel = courseKey ? getCourseLabel(courseKey) : t('course_none');
    const courseClass = courseKey ? 'course-badge' : 'course-badge empty';
    ...
    tr.innerHTML = `
      ...
      <td><span class="${courseClass}">${courseLabel}</span></td>
      <td style="font-family:var(--mono)">*****</td>
      <td>${faceStatus}</td>
      <td style="font-family:var(--mono)">+${ka} UZS</td>
  ...
  Array.from(tbody.querySelectorAll('tr td:nth-child(6) span')).forEach(el=>{
...
async function addEmp() {
  ...
  const kp = document.getElementById('ne_k').value;
  const courseKey = document.getElementById('ne_course')?.value || '';
  ...
  try {
    ...
    if (normalizedCourseKey && createdId) {
      await updateEmployeeCourseResponsibility(createdId, normalizedCourseKey);
    }
    let emps = await loadEmpList();
    if (normalizedCourseKey && !createdId) {
      const created = (emps || []).find(emp =>
        normalizeEmployeeName(emp.login) === normalizeEmployeeName(email) ||
        normalizeEmployeeName(emp.name) === normalizeEmployeeName(nm)
      );
      if (created?.id) {
        await updateEmployeeCourseResponsibility(created.id, normalizedCourseKey);
        emps = await loadEmpList();
      }
    }
    const courseSelect = document.getElementById('ne_course');
